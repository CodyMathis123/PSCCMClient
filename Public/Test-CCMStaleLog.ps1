    ...
    process {
        foreach ($Connection in (Get-Variable -Name $PSCmdlet.ParameterSetName -ValueOnly)) {
            ...
            if ($null -ne $RequestedLogInfo) {
                [datetime]$LogLastWriteTime = $RequestedLogInfo.LastModified
                ...
                Write-Warning $([string]::Format('{0} is missing; checking for recent ccmsetup activity', $LogFileName))
                ...
                if ($null -ne $MEMCMClientInstallLogInfo) {
                    [datetime]$dtmMEMCMClientInstallLogDate = $MEMCMClientInstallLogInfo.LastModified
